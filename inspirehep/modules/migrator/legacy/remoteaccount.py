...
def dump(u, from_date, **kwargs):
    ...
    try:
        ...
        current_app.logger.info(
            'user id {} has no remote account'.format(u.id))
    except MultipleResultsFound:
        current_app.logger.warn(
            'user id {} has multiple remote account'.format(u.id))
    ...
    try:
        ...
        current_app.logger.info('user id {} has no userext entry'.format(u.id))
    except MultipleResultsFound:
        current_app.logger.warn(
            'user id {} has multiple userext entries'.format(u.id))
